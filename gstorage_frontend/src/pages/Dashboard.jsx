...
export default function Dashboard() {
  document.title = "Gráficos - GStorage";
            ...
            {alertas.length === 0 ? (
              ...
              <div className="space-y-3">
                {alertas.map((alerta, i) => (
                  <div key={i} className={`p-3 rounded-lg border text-sm flex gap-3 items-start ${alerta.tipo === 'critico' ? 'bg-red-50 border-red-100 text-red-800' : 'bg-amber-50 border-amber-100 text-amber-800'
                    }`}>
                ...
                {despachos_list.length === 0 ? (
                  ...
                  despachos_list.slice(0, 5).map((d) => (
                    <tr key={d.id} className="hover:bg-slate-50 transition-colors group cursor-pointer">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider ${d.estado === 'En Carga' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-emerald-100 text-emerald-700 border border-emerald-200'
                          }`}>
                          {d.estado}
                        </span>
                      </td>
